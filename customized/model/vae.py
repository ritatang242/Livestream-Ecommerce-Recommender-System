...
#         loss = BCE + KLD
    ...
    def mytrain(self, input_full_context):
        ...
        for batch_size, z_dim in itertools.product(batch_list, z_dims):
            ...
            loss_record[z_dim] = []
    #         loss_record["Train_"+str(z_dim)+str(batch_size)] = []
    #         loss_record["Validation (z="+str(z_dim)+")"] = []
            ...
            for epoch in range(num_epochs):
                ...
                loss_record[z_dim].append(loss.item())
    ...
    #                 loss_record["Validation (z="+str(z_dim)+")"].appen(loss.item())
            ...
            mylabels = ["z=20", "z=30", "z=50"]
            plt.plot(range(len(loss_record[z_dim])), loss_record[z_dim], ls='-', label=z_dim)
            plt.ylabel('Training Loss')
            plt.xlabel('Number of Epoches')
            plt.legend(labels=mylabels, bbox_to_anchor=(0.98, 0.98), loc=1, borderaxespad=0.)
            plt.gca().yaxis.set_major_formatter(StrMethodFormatter('{x:,.2f}'))
        plt.savefig(fname=current_path+'/fig/loss_vae_z_'+str(z_dim)+'batch_'+str(batch_size)+'.pdf', dpi=300, format='pdf', bbox_inches='tight')
        plt.show()
