...
class LERS_without(nn.Module):
    def __init__(self, full_context_dim=408, z_dim=20, in_dim=811, h1_dim=256, h2_dim=64, out_dim=1): 
        super(LERS_without, self).__init__()
...
def run(cust_num, prod_num, streamer_product, rewards_df, z_dim=20, pos_weight=0):
    ...
    end2end = LERS_without(full_context_dim=full_context_dim, z_dim=z_dim, in_dim=in_dim, h1_dim=h1_dim, h2_dim=h2_dim, out_dim=out_dim).to(device)
...
#             print("change full context Double to Float")
        ...
        # backward (loss) with early stop
        the_current_loss = loss_funtion(pred_scores, context_rewards) # 每個商品都計算loss -> (1023, )
        last_loss_avg.append(the_current_loss.item()) # 為了做平均要記錄所有的loss -> (2602, 1023)
        if the_current_loss > np.mean(last_loss_avg[-3:]): # 取最後三次(3個人的全部商品loss)做移動平均 -> np.mean(3, 1023)
            trigger_times += 1
            signal = False
