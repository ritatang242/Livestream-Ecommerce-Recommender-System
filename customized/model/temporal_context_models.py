...
class CustomerStreamer():
    ...
    def train(self):
        ...
        for b,t in itertools.product(batch_list, t_list):
            ...
            cs_loss_record[str(t)+" Sequences Training Loss"] = []
            cs_loss_record[str(t)+" Sequences Validation Loss"] = []
            cs_epoch_loss[str(t)+" Sequences Training Loss"] = []
            ...
            for epoch in range(num_epochs):
                for i, (inputs, labels) in enumerate(train_loader): # 共 n_total_steps * N 人 -> 迭代 n_total_steps 次, 一批次 N 人
                    ...
                    cs_loss_record[str(t)+" Sequences Training Loss"].append(loss.item())
                    ...
                    if (i+1) % n_total_steps == 0:
                        cs_epoch_loss[str(t)+" Sequences Training Loss"].append(loss.item())
                        if (epoch+1) % 5 == 0:
...
#             torch.save(cs_model.state_dict(), current_path+'/customized/model/trained_model/customer_streamer/gru_'+str(b)+'_'+str(t)+'.pth')
            ...
            with torch.no_grad():
                ...
                for i, (inputs, labels) in enumerate(test_loader):
                    ...
                    cs_loss_record[str(t)+" Sequences Validation Loss"].append(loss.item())
...
class CustomerProduct(CustomerStreamer):
    ...
    def train(self):
        ...
        for b,t in itertools.product(batch_list, t_list):
            ...
            cp_loss_record[str(t)+" Sequences Training Loss"] = []
            cp_loss_record[str(t)+" Sequences Validation Loss"] = []
            cp_epoch_loss[str(t)+" Sequences Training Loss"] = []
...
#             loss_funtion = nn.BCEWithLogitsLoss() # BCE + sigmoid
            ...
            for epoch in range(num_epochs):
                for i, (inputs, targets) in enumerate(train_loader): # 不是labels而是targets
                    ...
                    cp_loss_record[str(t)+" Sequences Training Loss"].append(loss.item())
                    ...
                    if (i+1) % n_total_steps == 0:
                        cp_epoch_loss[str(t)+" Sequences Training Loss"].append(loss.item())
                        if (epoch+1) % 5 == 0:
                            print (f'Epoch [{epoch+1}/{num_epochs}], Step [{i+1}/{n_total_steps}], Loss: {loss.item():.4f}')
...
#             torch.save(cp_model.state_dict(), current_path+'/customized/model/trained_model/customer_product/gru_'+str(b)+'_'+str(t)+'.pth')
            ...
            with torch.no_grad():
                for i, (inputs, targets) in enumerate(test_loader):
                    ...
                    loss = loss_funtion(outputs, targets)
                    cp_loss_record[str(t)+" Sequences Validation Loss"].append(loss.item())
                    if (i+1) % n_total_steps == 0:
                        print (f'Step [{i+1}/{n_total_steps}], Loss: {loss.item():.4f}')        
