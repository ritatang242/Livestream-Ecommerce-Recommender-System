...
class LERS_both(nn.Module):
    def __init__(self, full_context_dim=408, vae_h_dim=256, z_dim=20, weight_eps=2, in_dim=811, h1_dim=256, h2_dim=64, out_dim=1): # 20customer+768product+23streamer 每一輪丟(一個人 配 一個商品)
        super(LERS_both, self).__init__()
...
def run(cust_num, prod_num, streamer_product, rewards_df, z_dim=20, weight_eps=2, pos_weight=0):
    ...
    end2end = LERS_both(full_context_dim=full_context_dim, vae_h_dim=vae_h_dim, z_dim=z_dim, weight_eps=weight_eps, in_dim=in_dim, h1_dim=h1_dim, h2_dim=h2_dim, out_dim=out_dim).to(device)
...
#         print(f"streamer_hn: {streamer_hn.shape}")
...
#         print(f"basket_hn: {basket_hn.shape}")

        # full context 
#         print(f"static_context: {static_context[t].shape}")
...
#         print(f"full_context: {full_context.shape}")
...
#             print("change full context Double to Float")
        ...
        blurry_context = z.detach().squeeze().cpu().numpy()  
...
# #     for k in range(10):
#     plt.plot(range(cust_num), hr, ls='-')
# #         plt.plot(range(cust_num), hr, ls='-', label=k)
#     plt.ylabel('HR@20 (%)')
#     plt.xlabel('Number of rounds')
# #         plt.legend(labels=[str(k)+"% of products" for k in range(20, 60, 20)],bbox_to_anchor=(.02, .98), loc=2, borderaxespad=0.)
#     plt.savefig(fname='./fig/hr_cust_test.pdf', dpi=300, format='pdf', bbox_inches='tight')
#     plt.show()
