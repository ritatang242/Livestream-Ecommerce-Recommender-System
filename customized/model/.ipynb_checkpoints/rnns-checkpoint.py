...
class GRU(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, output_size): # input_size: num_features (items or streamer)
        super(GRU, self).__init__()
        self.num_layers = num_layers
        self.hidden_size = hidden_size
        # -> x needs to be: (batch_size, seq, input_size)

        self.gru = nn.GRU(input_size, hidden_size, num_layers, batch_first=True) # seq_len cells
        # fully connected
        self.fc = nn.Linear(hidden_size, output_size) # many to one
...
class BiGRU(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, output_size): # input_size: num_features (items or streamer)
        ...
        self.gru = nn.GRU(input_size, hidden_size, num_layers, batch_first=True, bidirectional=True) # seq_len cells
        # fully connected
        self.fc = nn.Linear(hidden_size*2, output_size) # many to one

    def forward(self, x): # x會吃images
        # Set initial hidden states
        h0 = torch.zeros(self.num_layers*2, x.size(0), self.hidden_size).to(device) # x.size(0) equals to x.shape(0), namely "batch size of the input"
        # Intiate by random weights
        # h0 = torch.randn(self.num_layers, x.size(0), self.hidden_size).to(device)
...
class BayesGRU(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, output_size): # input_size: num_features (items or streamer)
        ...
        # bnn
        self.bnn = bnn.BayesLinear(prior_mu=0, prior_sigma=0.05, in_features=hidden_size, out_features=output_size)
