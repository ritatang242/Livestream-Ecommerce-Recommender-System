...
class UCB1(BaseBandit):
    ...
    def __init__(self, history_storage, model_storage, action_storage,
        ...
        for action_id in self._action_storage.iterids():
            total_action_reward[action_id] = 0.0
            action_times[action_id] = 0
    ...
    def _ucb1_score(self):
        ...
        score_dict = {}
        for action_id in self._action_storage.iterids():
            if (action_times[action_id] > 0):  # 2021/03/28 Rita fixed
                estimated_reward = (total_action_reward[action_id] # 印出
                                    / action_times[action_id])
                uncertainty = np.sqrt(2 * np.log(n_rounds)         # 印出
                                      / action_times[action_id])
                estimated_reward_dict[action_id] = estimated_reward
                uncertainty_dict[action_id] = uncertainty
                score_dict[action_id] = estimated_reward + uncertainty # 總和排序
            else: # 2021/03/28 Rita fixed
                estimated_reward_dict[action_id] = 0
                uncertainty_dict[action_id] = 1e400
                score_dict[action_id] = 1e400
        return estimated_reward_dict, uncertainty_dict, score_dict, total_action_reward, action_times # 第1,2,3名的前項、後項是多少 # 2021/02/20 action相關

    def get_action(self, context=None, n_actions=None): #  full context, 
        ...
        estimated_reward, uncertainty, score, total_action_reward, action_times = self._ucb1_score() # 2021/02/20
        if n_actions == -1:
...
#         return history_id, recommendations
        return history_id, recommendations, score, estimated_reward, uncertainty, total_action_reward, action_times  # 2021/02/07 Rita # 02/20

    def reward(self, history_id, rewards):  # 在policy_evaluation裡面有呼叫
